...
<script>
    $(document).ready(function() {
        $init_qty = Number($('.qty').val())
        $item_total = Number(net_price) * $qty;
        $total = 50 + $item_total;
        $('.item_total').val($item_total)
        $('.total').val($total)
    })

    function qtyIncr(product_id, net_price) {
        $qty = Number($('.qty').val())
        if ($qty < 4) {
            $qty += 1
            $('.qty').val($qty)
            ...
            $('.total').val($total)
            $.ajax({
                url: 'set_cart_product_incr_dicr.php',
                method: 'POST',
                data: {
            ...
            alert('You can Buy maximum 4 quantity at one order')
        }
    }

    function qtyDicr(product_id, net_price) {
        $qty = Number($('.qty').val())
        if ($qty > 1) {
            $qty -= 1
            $('.qty').val($qty)
                ...
                ?>
                                    <form action="cart.php?action=remove&product_id_array=$product_id_array" method="post">
                                        <tr>
                                            ...
                                            <td>
                                                <b><span class="qty_incr_dicr qty_dicr" onclick="qtyDicr('<?php echo $product_id; ?>', '<?php echo $row['net_price']; ?>')">-</span>&nbsp<input class="qty" type="text" name="qty" value=<?php echo $quantity; ?> readonly>&nbsp<span class="qty_incr_dicr qty_incr" onclick="qtyIncr('<?php echo $product_id; ?>', '<?php echo $row['net_price']; ?>')">+</span></b>
                                            </td>
                                            <td>
                                                <b><?php echo $size; ?></b>
                                            </td>
            ...
            <table class="table table-borderless">
                <tbody>
                    <tr>
                        ...
                        <td>Rs <input class="item_total" type="text" value="<?php echo $itemtotal; ?>" name="item_total" readonly></td>
                    </tr>
                    <tr>
                        <td>
                            <b>Delivery</b>
                        </td>
                        <td>Rs <input class="delivery" type="text" value="<?php echo $delivery; ?>" name="delivery" readonly></td>
                    </tr>
                    <tr class="table-active">
                        <td><b>Total</b></td>
                        <td>Rs <input class="total" type="text" value="<?php echo $total; ?>" name="total" readonly></td>
                    </tr>
                </tbody>
            </table>
            <ul>
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-4">
                                            ...
                                            ?>
                                                <img src="images/<?php echo $rowimg['img']; ?>" alt="<?php echo $rowimg['img']; ?>">
                                            <?php } ?>
                                            <div class="product-quantity">Qty- <span class="qty_incr_dicr qty_dicr" onclick="qtyDicr('<?php echo $product_id; ?>', '<?php echo $row['net_price']; ?>')">-</span>&nbsp<input class="qty" type="text" name="qty" value=<?php echo $quantity; ?> readonly>&nbsp<span class="qty_incr_dicr qty_incr" onclick="qtyIncr('<?php echo $product_id; ?>', '<?php echo $row['net_price']; ?>')">+</span></div>
                                        </div>
                                    </div>
...
if (isset($_POST["checkout"])) {
    ...
    if ($result->num_rows > 0) {
        ...
        if ($count > 0) {
            echo "<script>window.location.href='payuform.php?itemtotal=$_POST[item_total]&total=" . urlencode($_POST["total"]) . "'</script>";
        } else {
