...
<input type="text" id="searchTerm" onkeyup="doSearch()" placeholder="Search Any fields within table" class="form-control"/>				
		...
		<form method="post" class="form"><div class="col-md-3">
					<label>Search From</label>
		<input type="text" id="date1" name="dat1"class="form-control">
</div>
<div class="col-md-3">
					<label>Search To</label>
		<input type="text" id="date2" name="dat2" class="form-control">
</div>
<div class="col-md-2">					

		<input type="submit" class="btn btn-info" id="" name="sub" value="SUBMIT" style="margin-top:20px;">
</div></form>
...
$offset = ($currentpage - 1) * $rowsperpage;
	...
	$date1=date("Y-m-d",strtotime($_POST['dat1']));
	$date2=date("Y-m-d",strtotime($_POST['dat2']));
		$from=$date1." "."00:00:00";
	$to=$date2." "."23:59:59";
	
$sq="select * from place_order where date between '$from' and '$to' order by id desc LIMIT $offset, $rowsperpage";
...
$offset = ($currentpage - 1) * $rowsperpage;
$count=1;
$sq="select * from place_order order by id desc LIMIT $offset, $rowsperpage";
$r=mysqli_query($con,$sq);
while($row=mysqli_fetch_array($r)){
	//$status=$row['status'];
$pro_id=$row['order_id'];
